...
const createSegments = input => {
  ...
  commands.forEach(command => {
    const segment = segmentFromCommand(command, currentPoint);
    currentPoint =
      segment.pointA.x === currentPoint.x && segment.pointA.y === currentPoint.y
        ? { ...segment.pointB }
        : { ...segment.pointA };
...
const segmentFromCommand = (command, origin) => {
  const direction = command[0];
  const length = Number(command.substring(1));
  switch (direction) {
    case "L":
      return new Segment(origin, new Point(origin.x - length, origin.y));
    case "R":
      return new Segment(origin, new Point(origin.x + length, origin.y));
    case "U":
      return new Segment(origin, new Point(origin.x, origin.y + length));
    case "D":
      return new Segment(origin, new Point(origin.x, origin.y - length));
  }
};
  ...
  segments1.forEach( s1 => {
    segments2.forEach(s2 => {
      if (intersect(s1, s2)) {
        ...
        distances.push(Math.abs(intersection.x) + Math.abs(intersection.y));
      }
    });
  });
...
const intersect = (segment1, segment2) => {
  ...
  if (
    segment1.pointA.x == segment1.pointB.x &&
    segment2.pointA.y == segment2.pointB.y
  ) {
    // if the vertical line's X falls between the horizontal's ends and if the horizontal's
    // Y falls between the vertical's Y, then they intersect
    if (
      segment1.pointA.x.between(segment2.pointA.x, segment2.pointB.x) &&
      segment2.pointA.y.between(segment1.pointA.y, segment1.pointB.y)
    ) {
      return new Point(segment1.pointA.x, segment2.pointB.y);
    }
  }

  // these are perpendicular lines, segment2 being vertical | and segment1 being horizontal -
  if (
    segment2.pointA.x == segment2.pointB.x &&
    segment1.pointA.y == segment1.pointB.y
  ) {
    // if the vertical line's X falls between the horizontal's ends and if the horizontal's
    // Y falls between the vertical's Y, then they intersect
    if (
      segment2.pointA.x.between(segment1.pointA.x, segment1.pointB.x) &&
      segment1.pointA.y.between(segment2.pointA.y, segment2.pointB.y)
    ) {
      return new Point(segment2.pointA.x, segment1.pointB.y);
    }
  }
  return false;
};
